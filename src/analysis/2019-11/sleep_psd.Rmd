...
psd.molten = melt.freq.bands(psddf, extra.id.vars = c())
...
animal.psd.ratio %>%
  ggplot() +
  geom_line(aes(x=band_start_freq, group=animal, y=ratio.mean)) + 
...
daily.psd %>%
  ...
  geom_rect(data=freq.bands.df, mapping=aes(xmin=xmin, xmax=xmax, group=band), 
            ymin=-15, ymax=-8, alpha=0.05, fill='grey10') +
  facet_grid(. ~ channelLocation) +
  scale_x_log10(limits=c(1,200), breaks=axis.breaks, labels=axis.labels) +
  gtheme +
...
df.norm %>%
  ggpubr::ggboxplot(x='band', y='norm.value', color='channelLocation') +
  geom_hline(yintercept=1, linetype='dashed') +
...
animal.wcoh.df = wcoh.molten %>%
  dplyr::group_by(animal, band_name, band_start_freq) %>%
  dplyr::summarise(ratio.mean=mean(power.ratio.change, na.rm=TRUE),
                   ratio.sem=sem(power.ratio.change))
