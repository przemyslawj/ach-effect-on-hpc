...
freq.bands.df = data.frame(
  band=c('theta', 'slow gamma', 'med gamma', 'fast gamma', 'sm_gamma'),
  xmin=c(5, 25, 55, 120, 25),
  xmax=c(11, 45, 80, 250, 80)
)
```
...
joined.psd.fit.df = map_dfr(unique(animals.psd$animal), ~ {
  ...
  animal.fit.low.freq = calc.animal.band.fit.df(animal.psd, c(1, 20)) %>%
...
```{r}
bs.fit = filter(joined.psd.fit.df, name != 'bg_high_freq') %>%
  filter(animal == 'BS')
bind_rows(animal.psd, bs.fit) %>%
  ...
  geom_ribbon(aes(x=band_start_freq, y=band_power, 
                  ...
                  ymin=band_power - sem.power, ymax=band_power + sem.power), 
              alpha=0.2) +
  scale_x_log10(limits=c(2,19), breaks=axis.breaks, labels=axis.labels) +
  ylim(-12,-7.1)+
  scale_color_manual(values = c(nolaser.col, laser.col)) +
  scale_fill_manual(values = c(nolaser.col, laser.col)) +
...
```{r}
#fit.psd.df = fit.background.df(psd.molten, freq.bands.df, c(15, 120), fit.knee = FALSE)
...
g.sgamma.pow = plot.laser.change(fit.psd.df, !!var) + ylab('Relative Slow Gamma Power (20 - 45 Hz; a.u.)')
g.sgamma.pow
calc.laser.fixed.effects(subset(fit.sgamma.present, channelLocation == 'CA1'), !!var)
```
SGamma central frequency when the laser on...
```{r}
var = quo(peak_sm_gamma)
fit.sgamma.present %>%
  group_by(channelLocation, laserOn, animal) %>%
  dplyr::summarise(peak.mean = mean(!!var, na.rm=T),
                   peak.median = median(!!var, na.rm=T),
                   peak.sem = sem(!!var))

plot.laser.change(fit.psd.df, !!var) + ylab('Central Slow Gamma frequency (Hz)')
calc.laser.fixed.effects(filter(fit.sgamma.present, channelLocation == 'CA3', !is.na(!!var)), !!var)
```
## Theta
Fit background with FOOOF and check quality
```{r}
