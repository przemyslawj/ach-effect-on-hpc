...
```{r include=FALSE}
if (diode.signal) {
  ripple.df = read_csv(paste0(data_dir,'/','theta_ripples_diode_th6.csv'))
...
CA3
```{r}
ca3.psddf = filter(psddf, channelLocation == 'CA1')
m.full = lmerTest::lmer(log(0.001 + swr_incidence) ~ laserOn + (1 + laserOn | animal), data=ca3.psddf, REML = TRUE) 
