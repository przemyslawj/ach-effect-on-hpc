...
if (diode.signal) {
  #psddf.ymaze = read_csv(paste0(data_dir,'/','psd_table_diode_single_std_th6.csv'))
  psddf.ymaze = read_csv(paste0(data_dir,'/','psd_table_diode_th6.csv'))
  ripples.ymaze = read_csv(paste0(data_dir,'/','ripples_diode_th6.csv'))
} else {
  psddf.ymaze = read_csv(paste0(data_dir,'/','psd_table_th5.csv'))
...
#psddf.ymaze.correct %>%
  ...
  ggplot() +
  facet_grid(date ~ .) +
  geom_point(aes(x=stage_desc, y=has_ripples.pct, color=stage_desc))
...
[] Replicate figure: https://www.nature.com/articles/s41593-017-0061-5/figures/3
   - location of SWRs: x - time, y- # trial, point for ripple
   - Spectrogram on ymaze? avg for a day
[] Ripples correlation with success??
[] 20-50Hz oscillations at reward for CA3
