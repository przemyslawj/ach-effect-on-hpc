...
ttestchannel = function(gathered.psddf, experiments_tested, ch) {
  ...
  for (varname in varnames) {
    ...
    if (varname != 'ohas_ripples' && length(x) > 0) {
      ...
      if (!is.nan(tresult$p.value) & tresult$p.value < 0.05) {
...
for (varname in varnames) {
  ...
  if (varname != 'ohas_ripples' && length(x) > 0) {
    ...
    tresult = t.test(ondat[[varname]], offdat[[varname]], paired=TRUE)
    if (!is.nan(tresult$p.value) & tresult$p.value < 0.1) {
...
experiments_tested = best.gathered.psddf %>%
  filter(laser == 'ON') %>%
  filter(opow_theta > 0.000001) %>%
  filter(state == 'rest' | state == 'asleep')

ttestdat = subset(best.gathered.psddf,
  (experiment %in% experiments_tested$experiment) &
  (laser == 'ON' | time_sec == 0))

start.albert.swr_freq = subset(albert.pow_theta, laser == 'OFF' & time_sec == 0, oswr_freq, drop=TRUE)
ondat = filter(ttestdat, laser == 'ON')
rest.ondat = filter(ondat, state == 'rest')
asleep.ondat = filter(ondat, state == 'asleep')
offdat = filter(ttestdat, laser == 'OFF')
rest.offdat = filter(offdat, state == 'rest')
asleep.offdat = filter(offdat, state == 'asleep')

g1 = ggplot() +
  geom_boxplot(data=ttestdat, aes(x=state, y=oswr_freq, fill=laser)) +
  gtheme +
  xlab('') + ylab('Ripple incidence (Hz)') +
  theme(legend.position = 'none')

g2 = ggplot() +
  geom_boxplot(data=ttestdat, aes(x=state, y=oripple_dur, fill=laser)) +
  gtheme +
  xlab('') + ylab('Ripple duration (sec)') +
  theme(legend.position = 'none')

g3 = ggplot() +
  geom_boxplot(data=ttestdat, aes(x=state, y=oripple_peakpow, fill=laser)) +
  gtheme +
  xlab('') + ylab('Ripple peak power (a.u.)') +
  scale_fill_discrete(name = "Laser") +
  ylim(0,20)

combined_g = plot_grid(g1,g2,g3, labels = c('A','B','C'), nrow=1, ncol=3, label_size = 22, rel_widths = c(1,1,1.15))
combined_g
ggsave(file=paste0(img_dir, '/', 'rest_ripple_freq.svg'), plot=combined_g, width=11, height=4)

t.test(rest.offdat$oswr_freq, rest.ondat$oswr_freq, paired=T)
t.test(asleep.offdat$oswr_freq, asleep.ondat$oswr_freq, paired=T)

t.test(rest.offdat$oripple_dur, rest.ondat$oripple_dur, paired=T)
t.test(asleep.offdat$oripple_dur, asleep.ondat$oripple_dur, paired=T)
