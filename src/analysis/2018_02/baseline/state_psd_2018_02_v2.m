%% setup results
datarootdir = '/mnt/DATA/Prez/N&A_rest';

sleeping_trials_fpath = [datarootdir filesep 'state_v2.csv'];
...
for i = 1:nexp
    ...
    electrodes_file = '/mnt/DATA/Prez/N&A_rest/valid_electrodes.csv';
    ...
    filename_prefix = [expstable.experiment{i} '_'];
    ...
    rec_times = [0, expstable.startLaserSec(i);...
                 expstable.startLaserSec(i) + break_sec, expstable.stopLaserSec(i);...
                 expstable.stopLaserSec(i) + break_sec , total_sec];
    ...
    for channel = 1:nchans
        ...
        for rec_times_i = 1:size(rec_times, 1)
            ...
            psd_xx = downsampledDataArray(channel,...
                floor(sec_start * downfs) + 1 : floor(sec_end * downfs));
            ...
            for j=1:(numel(all_bands)-1)
             ...
             if ~isempty(section_ripples)
                ...
                section_ripples(:,1:3) = section_ripples(:,1:3) - expstable.startLaserSec(i);
