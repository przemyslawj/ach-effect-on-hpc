...
gbands = ggplot(animals.bands.df.filtered) +
  geom_line(aes(x=band, y=mchange, group=animal, colour=animal)) +
  ...
  #scale_color_manual(labels=c('Immobile w light / Immobile wo light', 'Mobile w light / Mobile wo light'), 
  #                   values=c(immobile.col, mobile.col, 'black')) +
...
```{r include=TRUE}
xlabels = rep(c('0 - 20 sec', '20 - 40 sec'), 2)
...
Experimental plot of decrease in ripples
```{r include=TRUE}
xlabels = rep(c('0 - 20 s', '20 - 40 s'), 2)
my_comparisons <- list( c("0", "1"))
...
paired.animal.summary %>%
  ...
  #geom_violin(aes(group=state_laser_num, colour=state), adjust=2,
  #            draw_quantiles = c(0.5)) +
  #geom_jitter(data=paired.state, aes(x=laserOn, y=swr_incidence), width=0.1) +
  #geom_dotplot(aes(fill=animal), binaxis = "y", stackdir = "center", 
  #             binwidth = 0.01, dotsize=0.8, stackratio=0.8) +
