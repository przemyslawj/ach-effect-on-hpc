...
compared.ripples.df %>%
  ...
  geom_violin() +
  facet_grid(stage_desc ~ exp + correct, scales = 'free') +
  ylab('Ripple incidence at goal (Hz)')
```
...
#compared.ripples.df.goal = filter(compared.ripples.df, stage_desc == 'DuringStim', laserOn == 0)
#m.full = lmerTest::lmer(swr_incidence ~ correct + (1 + correct | exp_animal), 
#                        data=compared.ripples.df.goal,
#                        REML = TRUE) 
#summary(m.full)
...
Model for ripples at start and goal between correct and incorrect trials
```{r}
m.full = lmerTest::lmer(has_swrs.pct ~ stage_desc * correct + (1 + stage_desc + correct | exp_animal), 
                        data=animal.summary.ripples,
                        REML = TRUE) 
summary(m.full)
print(anova(m.full, refit=FALSE, ddf='Satterthwaite'))
lsmeansLT(m.full, pairwise=TRUE)
plot.model.diagnostics(m.full, animal.summary.ripples$exp_animal, animal.summary.ripples$stage_desc)
create.animal.summary(animal.summary.ripples, has_swrs.pct, group_vars=vars(stage_desc, correct))

ggplot(animal.summary.ripples, aes(x=correct, y=100 * has_swrs.pct)) +
  #geom_point(aes(color=exp), shape=1, alpha=1.0, size=1.0, stroke=0.5) +
  geom_line(aes(group=exp_animal), color='#444444') +
  facet_grid(. ~ stage_desc, scales = 'free') +
  xlab('Successful') +
  ylab('Trials (%)') +
...
ggsave(paste0(img_dir, '/5-trials-with-ripples.pdf'), units = 'cm',
       dpi=300, device=cairo_pdf,
       height=4.0, width=4.3)
```
...
ca1.psddf %>%
  filter(laserOn == 0, stage_desc == 'DuringStim') %>%
  #group_by(exp, animal, exp_animal, learning_day) %>%
  #dplyr::summarise(m.swr_incidence=mean(swr_incidence),
  #                 sem.swr_incidence = sem(swr_incidence)) %>%
...
Peak frequency
```{r}
ca1.ripple.df = filter(ripples.ymaze, channelLocation == 'CA1', stage_desc == 'DuringStim', correct == 1) %>%
...
```{r}
m.full = calc.laser.fixed.effects(filter(ca1.ripple.df, peak_freq >= 140), peak_freq)
