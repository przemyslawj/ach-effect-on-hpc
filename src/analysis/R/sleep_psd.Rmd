...
if (diode.signal) {
  input.filename = 'welch_psd_table_diode_th6.csv'
} else {
  input.filename = 'welch_psd_table_th6.csv'
}
psddf = bind_rows(
...
animal.psd.ratio = psd.ratio.df %>%
  dplyr::group_by(animal, exp, band_name, band_start_freq, channelLocation, state) %>%
...
animal.psd.ratio %>%
  ...
  facet_grid(state + exp ~ channelLocation, scales = 'free_y') +
  xlab('Frequency (Hz)') +
...
channel.psd.ratio = psd.ratio.df %>%
  dplyr::group_by(band_name, band_start_freq, channelLocation, state, exp) %>%
  dplyr::summarise(ratio.mean=mean(power.ratio.change, na.rm=TRUE),
                   ratio.sem=sem(power.ratio.change))
...
channel.psd.ratio %>%
  ...
  scale_x_log10(limits=c(2,250), breaks=axis.breaks, labels=axis.labels) +
  geom_hline(yintercept=1, linetype='dashed') +
  facet_grid(state + exp ~ channelLocation) +
  xlab('Frequency (Hz)') +
  ylab('LFP power ratio') +
  gtheme +
...
ggsave(file=paste0(img_dir, '/', '2_psd_mean_all.pdf'), 
       width=19, height=31, unit='cm', dpi=300, device=cairo_pdf)
```
...
bind_rows(animals.psd, plotted.lowfreq.fit) %>%
  ...
  filter(animal == 'BD', exp == 'gfp-control') %>%
  dplyr::mutate(name_laser=paste(name, laserOn, sep='_')) %>%
  ggplot() +
  geom_line(aes(x=band_start_freq, y=band_power, 
                group=name_laser, color=laserOn, linetype=name)) +
...
trial.molten.psd = psd.molten %>%
  ...
  #filter(animal == 'BD', exp == 'gfp-control') %>%
  filter(animal == 'TR', exp == 'main-effect') %>%
...
```{r}
fit.psd.df = fit.background.df(psd.molten, freq.bands.df, c(15, 150), 
                               fit.knee = FALSE, 
...
pct.theta.oscillating = group_by(fit.psd.df, channelLocation, exp, animal, laserOn) %>%
...
Relative slow gamma power increased during the stimulation.  In the CA1, The change was significantly higher than in
the control animals not expressing Chr2 (ChAT-GFP animals). This we demonstrate by a significant interaction of the laser and mouse group
in the linear mixed-effects model.
In the CA3, the relative slow gamma power significantly increased as the effect of the laser activation(), but we did not find a 
significant difference between the laser effect on the GFP and Chr2-expressing mice (laser and mouse group interaction: p<)
```{r}
var = quo(pow_slow_gamma)
...
Individual trials
```{r}
peak_var = quo(pow_theta)
fit.psd.df %>%
  mutate(date=str_sub(trial_id, 1, 10)) %>%
#  psddf %>%
  filter(stage_desc != 'after_stim', exp == 'gfp-control') %>% 
  filter(animal == 'BD') %>%
  filter(!str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON2')) %>%
  filter(!str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON4')) %>%
  filter(!str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON6')) %>%
  filter(!str_starts(trial_id, '2020-07-27_BD_BlackDot_StimON1')) %>%
  #filter(!str_starts(trial_id, '2020-08-11_BR_BR_sleep_2_')) %>%
  ggplot(aes(x=laserOn, y=!!peak_var)) +
  geom_line(aes(group=trial_id)) +
  geom_point() +
  facet_grid(date ~ exp + channelLocation) + 
  gtheme

calc.laser.fixed.effects(
  filter(fit.theta.present, channelLocation == 'CA1', !is.na(pow_theta)), log(pow_theta))

calc.laser.fixed.effects(
  filter(fit.theta.present, channelLocation == 'CA1', !is.na(pow_theta),
         animal != 'BR',
        !str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON2'),
        !str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON4'),
        !str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON6'),
        !str_starts(trial_id, '2020-07-27_BD_BlackDot_StimON1')), log(pow_theta))
```

```{r}
fit.psd.df %>%
  filter(animal == 'BD', stage_desc != 'after_stim', exp == 'gfp-control') %>%
  filter(channelLocation == 'CA3') %>%
  ggplot() +
  geom_point(aes(x=trial_id, y=pow_theta, color=laserOn)) +
  facet_grid(channelLocation ~ .) +
  gtheme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ylab('SWR incidence (Hz)') + xlab('Trial id')
```
...
Fit background with FOOOF and check quality
```{r}
fit.psd.df = fit.background.df(psd.molten, freq.bands.df, c(4, 20),
                               fit.knee = FALSE,
                               extra.group.vars = c('exp_animal'))
...
g.theta.pow = plot.laser.change(fit.theta.present, !!var) + ylab('Relative Theta Power (5 - 11 Hz)')
g.theta.pow

excluded.trial_ids = c('2020-07-24_BD_BlackDot_Asleep_StimON2_0',
                       '2020-07-24_BD_BlackDot_Asleep_StimON4_3',
                       '2020-07-24_BD_BlackDot_Asleep_StimON2_1',
                       '2020-07-24_BD_BlackDot_Asleep_StimON6_3',
                       '2020-07-24_BD_BlackDot_Asleep_StimON3_0',
                       '2020-07-24_BD_BlackDot_Asleep_StimON6_4',
                       '2020-07-24_BD_BlackDot_Asleep_StimON2_3',
                       '2020-08-11_TR_TR_sleep_2_g0_1',
                       '2020-08-12_TL_TL_sleep_5_g0_2',
                       '2020-08-12_TR_TR_sleep_1_g0_1',
                       '2019-11-29_BS_BS_sleep_2_g0_0')

calc.laser.fixed.effects(filter(fit.theta.present, channelLocation == 'CA1',
                                !(trial_id %in% excluded.trial_ids),
                                #!str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON2'),
                                #!str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON4'),
                                #!str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON5'),
                                #!str_starts(trial_id, '2020-07-24_BD_BlackDot_Asleep_StimON6'),
                                #!str_starts(trial_id, '2020-07-27_BD_BlackDot_StimON1'),
                                animal != 'BR',
                                stage_desc != 'after_stim'), 
                         log(!!var))

# Calculates significance of effect on theta between gfp and chr2 groups
theta.change.df = fit.theta.present %>%
  filter(stage_desc != 'after_stim') %>%
  arrange(trial_id, channelLocation, stage_desc) %>%
  group_by(animal, exp, exp_animal, trial_id, channelLocation) %>%
  dplyr::summarise(theta.change=diff(pow_theta), n=n()) %>%
  filter(n>1)
theta.change.m = lmerTest::lmer(theta.change ~ 1 + exp + (1 | exp_animal),
                                data=filter(theta.change.df, channelLocation == 'CA1', !(trial_id %in% excluded.trial_ids)),
                                REML=TRUE)
anova(theta.change.m)

theta.change.df %>%
  ggplot(aes(x=animal, y=theta.change)) +
  geom_violin()+
  geom_hline(yintercept = 0, linetype='dashed') +
  facet_wrap(exp + channelLocation ~ .) +
  gtheme

calc.laser.fixed.effects(filter(fit.theta.present, channelLocation == 'CA1', stage_desc != 'after_stim'), log(!!var))

# Restricts calculation just to the main effect
calc.laser.fixed.effects( filter(fit.theta.present, !is.na(!!var), 
                                 channelLocation == 'CA1', exp=='main-effect',
                                 stage_desc != 'after_stim'), 
                          log(!!var),
                          fixed.effects.str = 'laserOn')

# Take into account within the trial correlations
calc.laser.fixed.effects( filter(fit.theta.present, !is.na(!!var), 
                                 channelLocation == 'CA1', 
                                 #!(trial_id %in% excluded.trial_ids),
                                 !str_starts(trial_id, '2020-07-24_BD'),
                                 stage_desc != 'after_stim'), 
                          !!var,
                          fixed.effects.str = 'laserOn*exp',
                          random.effects.str = '1 + laserOn + (1 + laserOn | exp_animal) + (1 | trial_id)')
m.full = lmerTest::lmer(pow_theta ~ 1 + laserOn + exp + laserOn:exp + (1 | trial_id) + (1 + laserOn | exp_animal), 
                        data=filter(fit.theta.present, !is.na(!!var),
                                    !(trial_id %in% excluded.trial_ids),
                                    channelLocation == 'CA1', 
                                    stage_desc != 'after_stim'),
                        REML = TRUE) 
...
fit.psd.df %>% group_by(channelLocation, laserOn, exp) %>% dplyr::summarise(mean(!!var, na.rm=T), sem(!!var))
```
