...
if (diode.signal) {
  input.filename = 'ripples_diode_th6.csv'
} else {
  input.filename = 'ripples_th6.csv'
}
ripple.df = bind_rows(
...
filter(ripple.df, !is.na(peak_freq), exp == 'main-effect')[,metric_name] %>% 
  pull-> ripple_freqs
...
m.full = lmerTest::lmer(swr_incidence ~ laserOn + exp + laserOn * exp + (1 + laserOn | exp_animal), 
                        data=ca1.psddf, REML = TRUE) 
...
```{r}
stage.name = 'stim'
filter(psddf, stage_desc != 'after_stim') %>%
  group_by(exp, animal, stage_desc, channelLocation) %>%
  dplyr::summarise(m.var=mean(swr_incidence)) %>%
  group_by(exp, stage_desc, channelLocation) %>%
  dplyr::summarise(mean(m.var), sem(m.var))

print('CA1')
summary(subset(ca1.psddf, stage_desc == stage.name & exp == 'main-effect')$swr_incidence) 
sem(subset(ca1.psddf, stage_desc == stage.name)$swr_incidence) 

print('CA3')
summary(subset(ca3.psddf, stage_desc == stage.name & exp == 'main-effect')$swr_incidence) 
...
m.full = lmerTest::lmer(peak_freq ~ laserOn + exp + laserOn * exp + (1 + laserOn | animal), 
                        data=ca3.ripple.df, REML = TRUE) 
...
Detected ripples are not shorter during the laser activation
```{r include=TRUE}
ripple.df = ripple.df %>%
...
kruskal.test(ripple_dur~laserOn, data=ripple.df, subset = exp == 'main-effect')
```
...
swr.baseline.summary = filter(psddf, stage_desc != 'after_stim') %>%
  group_by(exp, animal, laserOn, stage_desc, state, channelLocation) %>% 
...
swr.baseline.summary %>%
  ...
  facet_grid(. ~ channelLocation + exp) +
...
```{r}
psddf %>%
  filter(animal == 'BD', stage_desc != 'after_stim') %>%
  filter(channelLocation == 'CA3') %>%
  ggplot() +
  geom_point(aes(x=trial_id, y=swr_incidence, color=laserOn)) +
  facet_grid(channelLocation ~ .) +
  gtheme +
