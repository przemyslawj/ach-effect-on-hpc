...
```{r}
psddf = read_csv('psd_table_rest_trialduration60.csv')
...
#best.swr_channels$channel_i[1] = 16
#best.swr_channels$channel_i[2] = 16
...
albert.pow_theta =
  filter(paired.psddf, animal == 'Albert') %>%
  filter(state == 'asleep' )  %>%
...
ttestchannel = function(gathered.psddf, experiments_tested, ch) {
  ...
  for (varname in varnames) {
    ...
    if (varname != 'ohas_ripples' && length(x) > 0) {
      ...
      if (!is.nan(tresult$p.value) & tresult$p.value < 0.5) {
